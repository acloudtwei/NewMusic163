...
class NetEase(object):
    def __init__(self, username=''):
        self.header = {
            ...
            "X-Real-IP": "113.81.100.24",
            "User-Agent": "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_12_5) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/59.0.3071.115 Safari/537.36",
        }

        # 这里是获取配置文件的cookie数据
        with open('./config.json', 'r', encoding='utf-8') as f:
            self.COOKIE_DATA = json.loads(re.sub(r'\/\*[\s\S]*?\/', '', f.read()))["cookie"]
    ...
    # 这个创建cookie保存的数据个人认为可有可无,因为使用了cookie的话就没必要保存了
    def get_cookie_file(self, filename):
        data_dir = os.path.join(os.path.expanduser("."), ".user_data(存储cookie)")
    ...
    # 这个request是对网易云数据进行操作的接口，登录成功之后就会将登录信息存放在token上，这样就可以直接通过request去操作
    def request(self, method, path, params={}, base_url=BASE_URL, default={"code": -1}, custom_cookies={'os': 'pc'}):
        ...
        # 这里添加一下cookie,直接就可以用来!
        custom_cookies.update({'MUSIC_U': self.COOKIE_DATA})
    ...
    def login(self, username, password):
        ...
        if len(cookie_file) > 0:
            if self.username != username:
                # 这个是读取存储在文件的cookie，然后赋值给seesion保存
                cookie_jar = LWPCookieJar(cookie_file)
                cookie_jar.load()
                self.session.cookies = cookie_jar
                self.session.cookies.load()
