...
def start(event, context):
    ...
    for user_config in config['users']:
        ...
        if user_setting['WeCom']['enable'] and len(user_setting['WeCom']['corpid']) > 0 and len(
                user_setting['WeCom']['secret']) > 0 and len(user_setting['WeCom']['agentid']) > 0:
            alert = WeComAlert(
...
def main_handler(event, context):
    ...
    if 'requestContext' in event:
        data = {
            ...
            "body": "<html lang=\"zh-CN\"><meta charset=\"utf-8\"><body><p>" + "<br />" + res.replace("\n",
                                                                                                      "<br />") + "</p></body></html>"
        }
        return data
